      ...
      <div className='p-4 border-b border-gray-200'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2'>
            <X
              onClick={() => { onClearFilter(); setShowFilterPhone(false); }}
      ...
      <div className='p-4 space-y-6 sm:max-h-[calc(100vh-200px)] overflow-y-scroll no-scrollbar'>
          ...
          {expandedSections.platform && (
            <div className="flex flex-col gap-2">
              {platform.map((platform) => (
                <label key={platform.value} className="flex items-center text-gray-700 gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={filter.platform?.includes(platform.value) || false}
                    onChange={(e) => {
                      const checked = e.target.checked
                      const current = filter.platform || []
                      const updated = checked ? [...current, platform.value] : current.filter((p) => p !== platform.value)
                      onFilterChange({ platform: updated.length > 0 ? updated : null })
                    }}
                  />
        ...
        <div>
          ...
          {expandedSections.followers && (
            <select
              ...
              onChange={(e) => onFilterChange({ minFollowers: parseInt(e.target.value, 10) || 0 })}
        ...
        <div>
          ...
          {expandedSections.niche && (
            <select
              value={filter.niche || ""}
              onChange={(e) => onFilterChange({ niche: e.target.value || null })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-700 outline-indigo-500"
            >
